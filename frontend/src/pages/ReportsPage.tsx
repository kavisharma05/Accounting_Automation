...
export function ReportsPage() {
  ...
  async function loadSummary() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Failed to load GST summary");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1>Reports</h1>
          <p>What the CA needs — ledger and GST worksheet. You do not file from here.</p>
        </div>
        <button type="button" className="btn btn-primary" onClick={() => void handleLedger()}>
          Send ledger to CA
        </button>
      </div>
      ...
      <div className="toolbar">
        ...
        />
        <button type="button" className="btn btn-primary" onClick={() => void loadSummary()} disabled={loading}>
          {loading ? "Loading…" : "Load GST summary"}
        </button>
        <button type="button" className="btn btn-secondary" onClick={() => void handleGstrExport()}>
          Download GST Excel
        </button>
      </div>
      ...
      {summary ? (
        <>
          <div className="card-grid">
            <div className="stat-card">
              <div className="label">Sales (taxable)</div>
            ...
            <div className="stat-card">
              <div className="label">Net tax</div>
              <div className="value">{formatInr(summary.net_tax_payable)}</div>
            </div>
          </div>
          <div className="panel">
            <div className="panel-header">This period</div>
        ...
        <div className="empty-state">Pick dates and load the GST summary for your CA.</div>
      )}
    </>
  );
}
