...
**Login:** `admin@pilot.local` / the current pilot password
...
| 5 | 3:50 | Payments | **Record payment** → vendor from the bill → amount = cheat-sheet total → date = today → reference `UTRDEMO001` → apply to that purchase → **Save** → **Apply 194C** | “Payment knocks down the payable. TDS is computed on the payment, not typed into a spreadsheet.” |
...
| 8 | 6:20 | GSTR | Period = this month → **Load summary** → **Download Excel** | “GSTR-1 and 3B worksheets from posted invoices. Preparation only — the CA still files.” |
